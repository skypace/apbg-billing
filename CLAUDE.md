...
| Date | Version | Change |
|---|---|---|
| 2026-08-03 | onboarding retired | **Legacy QuestionScout customer-intake path removed.** Deleted `netlify/functions/onboard-customer.mjs` (unauthenticated QuestionScout webhook — flagged SECURITY TODO, no HMAC), `netlify/functions/approve-customer.mjs` (created customers **directly in QBO** then SF — contradicted the brix-order 1.34 single-writer decision, duplicate-QBO-customer risk), and `public/customer-approve.html` (+ a stray root `customer-approve.html` metadata artifact). The QuestionScout form's public front door was already dead: `brixbev.com/application` 404s and `alamedapointbeveragegroup.com` no longer resolves in DNS. New-customer intake is now exclusively **orders.brixbev.com/apply** → `orders.onboarding_requests` → brix-order `/admin/onboarding` (approve = SF customer + $0 setup ticket → QBO via SF sync → Finish = portal login). `createSFCustomer` in `sf-helpers.mjs` is now caller-less but kept (shared SF wrapper). Remaining manual steps: cancel the QuestionScout subscription; optionally add a Wix URL-redirect `/application` → `orders.brixbev.com/apply` (dashboard-only — Wix has no public redirect-manager API). |
