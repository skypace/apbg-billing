...
| 2026-07-09 | qbo writeback | **`qbo-cylinder-audit` edge function (v1) shipped.** Creates the $0 `CR-ADJ-<n>` cylinder audit adjustment invoice for brix-order's driver Cylinder Audit PWA: explicit DocNumber, one $0 line per count delta on the standard cylinder delivery items (CO8011 etc.) or `PU*` pickup items (negative-qty delivery fallback when no PU item exists, e.g. 10LB CO2), then an immediate ops-mirror upsert (`ops.qbo_invoices` + `ops.qbo_invoice_lines`, sync-qbo conflict keys) so brix-order's `v_cylinder_inventory` reflects the adjustment instantly. Same INTERNAL_PAY_SECRET + qbo_token lease pattern as `qbo-returned-payment`; read-only `preview` mode. Deployed to the shared Supabase project, verify_jwt=false. Manifest: new `qbo-cylinder-audit` multi-writer entry. |
| 2026-07-09 | qbo writeback | **`qbo-returned-payment` edge function (v1) shipped.** Books a returned/bounced customer payment for brix-order's admin "Record returned payment" action: voids the applied QBO Payment (invoice reopens) + creates the $35 "Returned Payment Fee" invoice (item/account find-or-created). Same INTERNAL_PAY_SECRET + qbo_token lease pattern as `qbo-charge`; read-only `preview` mode; refuses (needs_manual) when the payment spans multiple invoices. Deployed to the shared Supabase project, verify_jwt=false. |
