...
| Date | Version | Change |
|---|---|---|
| 2026-07-25 | shopify sync | **Shopify→QBO sync rebuilt in-house (`shopify-qbo-sync` edge function, deployed v1, INERT until go-live).** Intuit's QBO Commerce "Shopify channel" app was found duplicating documents with no idempotency key (inflated the Shopify clearing account 316 to $14,740.98 — 133 duplicate docs deleted 2026-07-25 with full JSON backup in `public.qbo_shopify_dedup_backup_20260725`; 6 closed-period 2024 docs need manual delete in the QBO UI; balance verified $660.54 after) AND booking every DTC sale to generic Service items so **inventory was never relieved** (~$97K retail since 2024-05 — the canned-product shortage mechanism). Replacement: paid Shopify order → Sales Receipt `SH-<order#>` (customer 1643, clearing 316) with **real SKU-mapped Inventory lines** via new `ops.shopify_item_map` (40 SKUs seeded; note Shopify `8PK6121` = QBO `8PK1621`); refunds → Refund Receipts (restocked lines restore qty); Shopify Payments payouts → Deposits to Chase 72 (net = bank credit, fee → account 318). Double idempotency (sync tables + QBO DocNumber lookup). New tables `ops.shopify_sync_orders/_refunds/_payouts/_config` (migration 20260725230450, RLS service-only, registered in sync-manifest) + `shopify_qbo_sync` check in `ops.fn_sync_health_extra()`. ⚠ Go-live (runbook in `supabase/functions/shopify-qbo-sync/README.md`): create custom Shopify app (read_orders + read_all_orders + read_shopify_payments_payouts), set `SHOPIFY_ADMIN_TOKEN` secret, **disconnect the Intuit Shopify channel first**, flip `ops.shopify_sync_config.enabled=true`, schedule the 30-min pg_cron, one-time physical-count inventory true-up at cutover. |
