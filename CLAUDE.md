...
| 2026-07-09 | qbo writeback | **`qbo-customer-lookup` edge function shipped (v1).** Live QBO Customer query by exact display name for brix-order's onboarding queue — `ops.qbo_customers` only refreshes daily (sync-qbo), which left "Finish onboarding" greyed up to ~24h after a new customer's first $0 setup invoice posted. On a hit the function upserts the customer into `ops.qbo_customers` (same conflict key as sync-qbo) so the mirror self-heals. Same INTERNAL_PAY_SECRET + token-lease pattern; verify_jwt=false. |
