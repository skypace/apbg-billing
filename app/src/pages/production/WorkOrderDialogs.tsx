...
// ── Change the plan quantity (20260911d) ─────────────────────────────────
// Sky, 2026-09-11: "I accidentally listed the June diet cola run as 1,500gal
// instead of 2,000gal and I can't update it." The dialog takes the new case
// count (or gallons — cases are what the work order is planned in, so gallons
// are converted through the BOM's finished volume per case and rounded UP to a
// whole case) and a reason, and says what the server will carry the change
// through to before the button is pressed.
export function RescaleDialog({ wo, busy, onCancel, onSubmit }: {
  wo: WorkOrderView; busy: boolean;
  onCancel: () => void;
  onSubmit: (qty: number, reason: string) => void;
}) {
  const oldQty = Number(wo.qty_to_produce);
  const galPerCase = oldQty > 0 && Number(wo.batch_size_gal ?? 0) > 0 ? Number(wo.batch_size_gal) / oldQty : null;
  const [qty, setQty] = useState(String(oldQty));
  const [gal, setGal] = useState(galPerCase ? String(Math.round(oldQty * galPerCase)) : '');
  const [reason, setReason] = useState('');
  const newQty = Number(qty);
  const valid = Number.isFinite(newQty) && newQty > 0 && newQty !== oldQty;
  const factor = valid ? newQty / oldQty : null;
  const started = wo.status === 'in_production';
  return (
    <div className="cd" style={{ padding: 12, marginTop: 12, border: '1px solid var(--ac)' }}>
      <div style={{ fontSize: 10.5, color: 'var(--mt)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>
        Change the plan quantity — {fmtNum(oldQty)} units today
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
        <LField label="New quantity (units / cases)">
          <input type="number" min={1} step="1" style={inp()} value={qty}
            onChange={(e) => { setQty(e.target.value); if (galPerCase && Number(e.target.value) > 0) setGal(String(Math.round(Number(e.target.value) * galPerCase))); }} />
        </LField>
        {galPerCase && (
          <LField label={`Or finished gallons (${galPerCase.toFixed(2)} gal per case)`}>
            <input type="number" min={1} step="any" style={inp()} value={gal}
              onChange={(e) => { setGal(e.target.value); const g = Number(e.target.value); if (g > 0) setQty(String(Math.ceil(g / galPerCase))); }} />
          </LField>
        )}
        <LField label="Reason (goes on the work order's history)">
          <input style={inp()} value={reason} placeholder="e.g. entered as 1,500 gal, should be 2,000 gal" onChange={(e) => setReason(e.target.value)} />
        </LField>
      </div>
      <div style={{ fontSize: 11, color: 'var(--mt)', marginTop: 10, lineHeight: 1.5 }}>
        {factor != null
          ? <>Every material, recipe line and open purchase-order line on this work order is scaled by <strong>×{factor.toFixed(4)}</strong>
              {galPerCase ? <> (≈ {fmtNum(Math.round(newQty * galPerCase))} gal finished)</> : null}.
              Flat per-run fees do not change.{started ? ' Stock already consumed at the co-packer is corrected with new ledger movements for the difference — nothing already posted is edited.' : ''}</>
          : 'Enter a different quantity.'}
        {' '}A purchase order already pushed to QuickBooks blocks this — edit that PO and push it first.
      </div>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 12 }}>
        <button style={btnSecondary()} onClick={onCancel}>Cancel</button>
        <button style={btnPrimary()} disabled={busy || !valid} onClick={() => onSubmit(newQty, reason.trim())}>
          Change to {valid ? fmtNum(newQty) : '…'} units
        </button>
      </div>
    </div>
  );
