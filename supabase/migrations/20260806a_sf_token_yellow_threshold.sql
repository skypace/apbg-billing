-- sf_token health: stop the 4×/day yellow flap. (Applied live 2026-08-06.)
--
-- The old yellow rule fired on `access_expires_at < now() - 2 hours`. SF access
-- tokens live ~1h and are refreshed ON DEMAND by whichever function needs them,
-- so an expired access token is the NORMAL resting state between cron runs. The
-- SF-touching crons are deliberately spaced ~6h apart (sf-receipt-sync-fresh at
-- 03/15/21 UTC, sf-job-sync 09:00) to respect SF rate limits, which meant the
-- check went yellow ~3h after every run and healed on the next one — 4 alert
-- emails + 4 "resolved" emails a day, none of them a real fault.
--
-- New yellow rule: the token row hasn't been WRITTEN in 8h. The widest normal
-- gap between SF cron runs is 6h, so 8h means a run was actually missed (the
-- failure we care about) with 2h of margin. Red rules are unchanged: a refresh
-- error newer than the last successful write, or no write in 30h.
