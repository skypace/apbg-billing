-- Margin health should follow the current QBO runner, not retired direct
-- pg_cron job names. Keep RPC result shapes stable for the app.
...
AS $$
  WITH raw_times AS (
    SELECT
      ...
      (SELECT max(completed_at) FROM ops.sync_log WHERE source = 'qbo' AND sync_type = 'lines_backfill' AND status IN ('success', 'ok')) AS last_missing_line_at,
      (SELECT max(completed_at) FROM ops.sync_log WHERE source = 'qbo' AND sync_type = 'runner_refresh_lines' AND status IN ('success', 'ok')) AS last_rolling_line_at,
  ...
  times AS (
    SELECT
      ...
      (
        SELECT max(ts)
        FROM (VALUES
          (raw_times.last_invoice_sync_at),
          (raw_times.last_missing_line_at),
          (raw_times.last_rolling_line_at)
        ) AS v(ts)
      ) AS last_line_backfill_at
    FROM raw_times
  ),
  checked AS (
    SELECT
      ...
      array_remove(ARRAY[
        ...
        CASE WHEN last_line_backfill_at IS NULL THEN 'No successful QBO invoice-line runner logged'
             WHEN last_line_backfill_at < now() - interval '2 hours' THEN 'QBO invoice-line runner is older than 2h' END,
...
AS $$
  ...
  sync_times AS (
    SELECT
      ...
      (
        SELECT max(ts)
        FROM (VALUES
          (sync_raw.last_invoice_sync_at),
          (sync_raw.last_missing_line_at),
          (sync_raw.last_rolling_line_at)
        ) AS v(ts)
      ) AS last_line_runner_at
    FROM sync_raw
  ),
  ...
  issues AS (
    SELECT
      ...
      'Check sync-qbo, QBO token refresh, and qbo-sync-runner before trusting fresh Margin data.'::text AS action
    FROM sync_errors
    WHERE line_count > 0
    ...
    SELECT 'sync_invoice_stale',
      ...
      'Inspect qbo-sync-runner invoice mode.'
    FROM sync_times
    WHERE last_invoice_sync_at IS NULL OR last_invoice_sync_at < now() - interval '30 hours'

    UNION ALL
    SELECT 'sync_line_runner_stale',
      CASE WHEN last_line_runner_at IS NULL OR last_line_runner_at < now() - interval '2 hours' THEN 'critical' ELSE 'warn' END,
      'Invoice-line runner stale',
      'Last successful QBO invoice-line runner: ' || COALESCE(to_char(last_line_runner_at, 'YYYY-MM-DD HH24:MI TZ'), 'never'),
      0::bigint, NULL::numeric, ARRAY['qbo-sync-runner']::text[],
      'Inspect qbo-sync-runner lines and refresh-lines modes.'
    FROM sync_times
    WHERE last_line_runner_at IS NULL OR last_line_runner_at < now() - interval '30 minutes'
